...
class AopCertClient
{
    ...
    private function issuersToString($issuers){
        $issuer_arr = [];
    ...
    public function getCertSN($certPath)
    {
        ...
        if (strpos($certdata['serialNumber'], '0x') === 0) {
            // 转换十进制表示
            $serialNumber = $this->bchexdec($certdata['serialNumber']);
        } else {
            $serialNumber = $certdata['serialNumber'];
        }
        // 拼装issuser串
        $issuer = $this->issuersToString($certdata['issuer']);
